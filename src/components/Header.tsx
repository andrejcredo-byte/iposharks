...
export const Header: React.FC<{ onMenuOpen: () => void }> = ({ onMenuOpen }) => {
  return (
    <header className="h-20 bg-shark-bg/80 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-40">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3 bg-brand-primary/10 text-brand-primary px-4 py-2 rounded-full border border-brand-primary/20">
          <CheckCircle size={16} />
          <span className="text-xs font-bold uppercase tracking-wider">Верифицирован</span>
        </div>
        
        <button className="relative p-2 text-gray-500 hover:text-brand-primary transition-colors">
          <Bell size={22} />
          <span className="absolute top-2 right-2 w-2 h-2 bg-brand-secondary rounded-full border-2 border-shark-bg"></span>
        </button>
        
        <div className="flex items-center gap-3 pl-6 border-l border-white/5">
          <div className="text-right">
            <p className="text-xs font-bold text-white">Shark Investor</p>
            <p className="text-[10px] text-gray-500 font-medium">ID: 777888</p>
          </div>
